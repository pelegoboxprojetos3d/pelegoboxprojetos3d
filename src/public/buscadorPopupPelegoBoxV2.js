...
(() => {
  ...
  function mount() {
    ...
    // Remove qualquer versão velha que tenha ficado viva numa navegação SPA.
    ...
    root.innerHTML = `
      <style>
        ...
        #${ROOT_ID} .pbx-brand b{color:#ff3647}
        #${ROOT_ID} .pbx-chevrons{font-size:27px;line-height:.7;font-weight:300}

        #${ROOT_ID} .pbx-panel{position:fixed;left:72px;top:50%;width:min(620px,calc(100vw - 92px));transform:translate(calc(-100% - 86px),-50%);opacity:0;visibility:hidden;pointer-events:auto;transition:transform .68s cubic-bezier(.2,.9,.25,1),opacity .22s ease;z-index:4}
        ...
        #${ROOT_ID} .pbx-input{min-width:0;flex:1;height:42px;border:0;outline:0;background:transparent;color:#0b1f55;font-size:17px;font-weight:600;padding:0 5px}
        ...
        #${ROOT_ID} .pbx-submit{height:40px;min-width:88px;padding:0 20px;border:0;border-radius:999px;background:linear-gradient(90deg,#185ff1,#1551d8);color:#fff;font-weight:800;cursor:pointer;box-shadow:0 6px 14px rgba(24,91,224,.25)}
